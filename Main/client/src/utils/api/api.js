...
export const getAllExplore = () => API.get("/explore");
...
export const uploadPost = async (userId, desc, img) => {
  ...
  return res.data; // gpt suggested 
}catch(e){
    console.log(e);
  }


  //return res.data;   // original version
};


export const uploadExploreFeed = async (userId, desc) => {
  try {
    const res = await API.post("/explore/create-explore", { userId, desc });
    return res.data;
  } catch (error) {
    console.error("Error uploading explore feed:", error);
    throw error;
  }
};
